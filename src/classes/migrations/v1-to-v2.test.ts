...
describe('V1 to V2 Class Tests', () => {
    ...
    test('Run Calendar Migration', () => {
        ...
        (<Mock>(<Game>game).settings.get)
            ...
            .mockReturnValueOnce([{name: 'January', numericRepresentation: 1, numericRepresentationOffset: 0, numberOfDays: 31, numberOfLeapYearDays: 31, intercalary: false, intercalaryInclude: false}]) //Months
            .mockReturnValueOnce([{name: 'Moon', cycleLength: 30, phases:[], firstNewMoon: {yearReset: MoonYearResetOptions.None, yearX: 0, year: 0, month: 0, day: 1}, color: '#ffffff', cycleDayAdjust: 0}]) //Moons
            .mockReturnValueOnce([]) //Note Categories
            .mockReturnValueOnce([{name: 'Spring', startingMonth: 0, startingDay: 1, color: '#fffce8'}, {name: 'Summer', startingMonth: 0, startingDay: 1, color: '#f3fff3'}, {name: 'Fall', startingMonth: 0, startingDay: 1, color: '#fff7f2'}, {name: 'Winter', startingMonth: 0, startingDay: 1, color: '#f2f8ff'}]) //Seasons
    ...
    test('Run Global Configuration Migration', () => {
        ...
        expect(SC.globalConfiguration.permissions.viewCalendar).toEqual({player: false, trustedPlayer: false, assistantGameMaster: false, users: undefined});
    ...
    test('Run Note Migration', async () => {
        ...
        //@ts-ignore
        jest.spyOn(NManager, 'createNote').mockImplementation(async () => {return je;});
        (<Mock>(<Game>game).settings.get).mockReturnValueOnce([{allDay: false, calendarId: '', repeats: 0, order: 0, categories: [], remindUsers: [], startDate: {year: 0, month: 1, day: 1, hour: 0, minute: 0, seconds: 0}, endDate: {year: 0, month: 1, day: 1, hour: 0, minute: 0, seconds: 0}}]); //Notes
        expect(await V1ToV2.runNoteMigration()).toBe(true);
        ...
        (<Mock>(<Game>game).settings.get).mockReturnValueOnce([{allDay: false, author: 'asd', calendarId: '', repeats: 0, order: 0, categories: [], remindUsers: [], startDate: {year: 0, month: 1, day: 1, hour: 0, minute: 0, seconds: 0}, endDate: {year: 0, month: 1, day: 1, hour: 0, minute: 0, seconds: 0}}]); //Notes
        expect(await V1ToV2.runNoteMigration()).toBe(true);
        expect(NManager.createNote).toHaveBeenCalledTimes(2);
        expect(je.update).toHaveBeenCalledTimes(2);

        (<Mock>(<Game>game).settings.get).mockReturnValueOnce([{allDay: false, author: 'aasd', playerVisible: true, calendarId: '', repeats: 0, order: 0, categories: [], remindUsers: [], startDate: {year: 0, month: 1, day: 1, hour: 0, minute: 0, seconds: 0}, endDate: {year: 0, month: 1, day: 1, hour: 0, minute: 0, seconds: 0}}]); //Notes
        expect(await V1ToV2.runNoteMigration()).toBe(true);
