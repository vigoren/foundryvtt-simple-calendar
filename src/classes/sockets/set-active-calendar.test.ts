...
describe('Set Active Calendar Socket Tests', () => {
    ...
    test('Process', async () => {
        ...
        expect(await s.process({type: SocketTypes.mainAppUpdate, data: {}}, tCal)).toBe(false);

        //@ts-ignore
        game.user.isGM = true;
        SC.primary = true;
        expect(await s.process({type: SocketTypes.setActiveCalendar, data: {calendarId: ''}}, tCal)).toBe(true);
