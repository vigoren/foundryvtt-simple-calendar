...
describe('Note Update Socket Tests', () => {
    ...
    test('Process', async () => {
        ...
        r = await s.process({type: SocketTypes.noteUpdate, data: {}}, tCal);
        expect(r).toBe(true);
        expect(GameSockets.emit).not.toHaveBeenCalled();
        expect(MainApplication.updateApp).not.toHaveBeenCalled();

        r = await s.process({type: SocketTypes.noteUpdate, data: {journalId: '123', userId: 'asd'}}, tCal);
        expect(r).toBe(true);
        expect(GameSockets.emit).toHaveBeenCalledTimes(1);
        expect(MainApplication.updateApp).not.toHaveBeenCalled();

        r = await s.process({type: SocketTypes.noteUpdate, data: {journalId: '123', userId: 'qwe'}}, tCal);
        expect(r).toBe(true);
        expect(GameSockets.emit).toHaveBeenCalledTimes(2);
        expect(MainApplication.updateApp).not.toHaveBeenCalled();

        r = await s.process({type: SocketTypes.noteUpdate, data: {calendarId: 'test', newOrder: ['1'], date: {year: 0, month: 0, day: 0, hour: 0, minute: 0, seconds: 0}}}, tCal);
        expect(r).toBe(true);
