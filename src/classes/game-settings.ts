...
export class GameSettings {
    ...
    static async SaveGeneralSettings(settings: GeneralSettings): Promise<boolean> {
        if(GameSettings.IsGm()){
            ...
            if(JSON.stringify(settings) !== JSON.stringify(currentSettings)){
                return (<Game>game).settings.set(ModuleName, SettingNames.GeneralConfiguration, settings).then(()=>{return true;});
            } else {
    ...
    static async SaveNoteCategories(categories: NoteCategory[]): Promise<any> {
        ...
        if(GameSettings.IsGm()){
            ...
            if(JSON.stringify(current) !== JSON.stringify(categories)){
                return (<Game>game).settings.set(ModuleName, SettingNames.NoteCategories, categories).then(() => {return true});
            }else {
    ...
    static async SetDefaultNoteVisibility(visibility: boolean): Promise<boolean>{
        if(GameSettings.IsGm()) {
            return await (<Game>game).settings.set(ModuleName, SettingNames.DefaultNoteVisibility, visibility).then(() => {return true;});
        }
        return false;
    }
