...
export function ParseChatCommand(message: string) {
    ...
    if(messageParts[0] === '/scal' && messageParts.length > 1){
        ...
        if(processed){
            ...
            content += `<div class="fsc-chat-command-result">${FormatDateTime(currentDate, `${calendar.generalSettings.dateFormat.date} ${calendar.generalSettings.dateFormat.time}`, calendar)}</div>`
            content += `<div class="fsc-actions"><button class="fsc-control" data-date='${JSON.stringify({year: currentDate.year, month: currentDate.month, day: currentDate.day})}'>View In Simple Calendar</button></div>`
            ChatMessage.create({
                type: 1,
