...
export class BM25Levenshtein {
    ...
    private addDocument(doc: SimpleCalendar.Search.Document){
        ...
        for(let k in internalDocument.terms){
    ...
    public tokenize(text: string){
        return text.toLowerCase().replace(/\r?\n|\r/g, ' ').trim().split(' ').filter(w => this.stopWords.indexOf(w) === -1);
    }
        ...
        for(let k in this.documents){
            ...
            for(let i = 0; i < queryTerms.length; i++){
                ...
                if (this.terms[queryTerm] && this.documents[k].terms[queryTerm]) {
                    ...
                    const denom = this.documents[k].terms[queryTerm].count + (this.k1 * (1 - this.b + (this.b * this.documents[k].termCount / this.averageDocumentTermCount)));
