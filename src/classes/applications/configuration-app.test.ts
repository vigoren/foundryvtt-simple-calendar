...
describe('Configuration App Class Tests', () => {
    ...
    beforeEach(async () => {
        ...
        jest.spyOn(CalManager, 'cloneCalendars').mockImplementation(() => {return [cloneCal];});
    ...
    test('Activate Listeners', () => {
        ...
        //@ts-ignore
        jest.spyOn(DateSelectorManager, 'GetSelector').mockImplementation(() => {return {activateListeners: jest.fn()};});
    ...
    test('Input Change', () => {
        //@ts-ignore
        jest.spyOn(ca, 'writeInputValuesToObjects').mockImplementation(() => {return true;});
        //@ts-ignore
        jest.spyOn(ca, 'updateUIFromObject').mockImplementation(() => {return true;});
        //@ts-ignore
        ca.inputChange();
        //@ts-ignore
    ...
    test('Calendar Click', () => {
        ...
        //@ts-ignore
        jest.spyOn(ca, 'updateApp').mockImplementation(() => {return true;});
    ...
    test('Add New Calendar',  async() => {
        ...
        jest.spyOn(PredefinedCalendar,'setToPredefined').mockImplementation(async () => {return true;});
        //@ts-ignore
        jest.spyOn(ca, 'updateApp').mockImplementation(() => {return true;});
    ...
    test('Date Selector Change', () => {
        ca.dateSelectorChange(tCal.seasons[0].id, ConfigurationDateSelectors.seasonStartingDate, {startDate:{month:0, day: 0}, endDate:{}, timeSelected: false});
        expect(tCal.seasons[0].startingMonth).toBe(0);
        expect(tCal.seasons[0].startingDay).toBe(0);

        ca.dateSelectorChange(tCal.seasons[0].id, ConfigurationDateSelectors.seasonStartingDate, {startDate:{month:1, day: 1}, endDate:{}, timeSelected: false});
        expect(tCal.seasons[0].startingMonth).toBe(1);
        expect(tCal.seasons[0].startingDay).toBe(1);

        ca.dateSelectorChange(tCal.seasons[0].id, ConfigurationDateSelectors.seasonSunriseSunsetTime, {startDate:{hour:0, minute: 0}, endDate:{hour:0, minute: 0}, timeSelected: true});
        expect(tCal.seasons[0].sunriseTime).toBe(0);
        expect(tCal.seasons[0].sunsetTime).toBe(0);

        ca.dateSelectorChange(tCal.seasons[0].id, ConfigurationDateSelectors.seasonSunriseSunsetTime, {startDate:{hour:1, minute: 1}, endDate:{hour:1, minute: 1}, timeSelected: true});
        expect(tCal.seasons[0].sunriseTime).toBe(3660);
        expect(tCal.seasons[0].sunsetTime).toBe(3660);

        ca.dateSelectorChange(tCal.moons[0].id, ConfigurationDateSelectors.moonFirstNewMoonDate, {startDate:{month:0, day: 0}, endDate:{}, timeSelected: false});
        expect(tCal.moons[0].firstNewMoon.month).toBe(0);
        expect(tCal.moons[0].firstNewMoon.day).toBe(0);

        ca.dateSelectorChange(tCal.moons[0].id, ConfigurationDateSelectors.moonFirstNewMoonDate, {startDate:{month:1, day: 1}, endDate:{}, timeSelected: false});
    ...
    test('Export Calendar', () => {
        ...
        //@ts-ignore
        jest.spyOn(game.journal, 'forEach').mockImplementation((v: any) => {return v.call(undefined, { getFlag: () => {return {calendarId: 'a'};}, toObject: () => {} })});
    ...
    test('Import Calendar Save', async () => {
        ...
        const data = {
            ...
            notes: {
                a: [{flags:{"foundryvtt-simple-calendar":{noteData:{calendarId: 'a'}}}},{flags:{"foundryvtt-simple-calendar":{noteData:{calendarId: 'a'}}}}],
                b: [{flags:{"foundryvtt-simple-calendar":{noteData:{calendarId: 'b'}}}},{flags:{"foundryvtt-simple-calendar":{noteData:{calendarId: 'b'}}}}]
            }
        };
