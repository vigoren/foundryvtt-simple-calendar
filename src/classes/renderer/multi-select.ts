...
export default class MultiSelect {
    ...
    public static Render(options: SimpleCalendar.Renderer.MultiSelectOptions = {id: '', options: []}, open: boolean = false): string{
        ...
        const selectedValues = options.options.filter(o => o.selected).map(o => o.value).join('|');
        const selectedText = options.options.filter(o => o.selected).map(o => o.text).join(', ') || 'None Selected';
        ...
        html += `<button><div class="fsc-selected-options">${selectedText}</div><i class="fa fa-chevron-down"></i></button><ul class="fsc-multi-select-options ${open? 'fsc-show' : ''}">`;
        for(let i = 0; i < options.options.length; i++){
            html += `<li class="${options.options[i].disabled? 'fsc-disabled' : ''} ${options.options[i].selected? 'fsc-selected' : ''}" data-value="${options.options[i].value}"><span class="fa-solid ${options.options[i].selected? 'fa-square-check' : 'fa-square'}"></span>${options.options[i].text}</li>`;
        }
        html += `</ul></div>`;
        return html;
    }

    public static ActivateListeners(multiSelectId: string, onOptionChange: Function){
        const multiSelect = document.getElementById(multiSelectId)?.parentElement;
        if(multiSelect){
            multiSelect.querySelector('button')?.addEventListener('click', MultiSelect.EventListener.bind(MultiSelect, multiSelectId, 'button', onOptionChange));
            multiSelect.querySelectorAll('li').forEach(e => {e.addEventListener('click', MultiSelect.EventListener.bind(MultiSelect, multiSelectId, 'option', onOptionChange));});
        }
    }

    public static EventListener(multiSelectId: string, type: string, onOptionChange: Function, event: Event){
        const multiSelect = document.getElementById(multiSelectId)?.parentElement;
        if(multiSelect){
            if(type === 'button'){
